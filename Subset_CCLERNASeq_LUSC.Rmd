...
CCLE_RNAseq_reads = read.csv("~/Project_LUSC/RScripts_LUSC/Resources/CCLEDownloads/CCLE_RNAseq_reads.csv", header=TRUE, stringsAsFactors = FALSE, row.names = 1 )

sample_meta = read.csv("~/Project_LUSC/RScripts_LUSC/Resources/CCLEDownloads/sample_info.csv", header = TRUE)

CCLEmutations = read.csv("~/Project_LUSC/RScripts_LUSC/Resources/CCLEDownloads/CCLE_mutations.csv", header = TRUE)
...
CCLEmutations_subset_tp53_df = CCLEmutations_subset_tp53[,c("DepMap_ID", "Protein_Change")]
  
sample_meta = left_join(sample_meta, CCLEmutations_subset_tp53_df)
colnames(sample_meta)[colnames(sample_meta) == "Protein_Change"] <- "p53"
...
sample_meta_LUSC = subset(sample_meta_LUSC, sample_meta_LUSC$stripped_cell_line_name %in% row.names(MatchedCellsTable))
